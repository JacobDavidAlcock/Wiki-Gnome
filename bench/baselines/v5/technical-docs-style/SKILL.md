...
- Describe what it returns, side effects such as writing files, and what happens in edge cases.
...
def withdraw(account: Account, amount: int) -> int:
    ...
    Args:
        ...
        amount: Amount to withdraw, in pence. Must be 0 or more; 0 changes
            nothing.

    Returns:
        The account's new balance, in pence.

    Raises:
        NegativeAmount: If amount is less than 0.
        InsufficientFunds: If amount is more than the balance. The balance
            is left unchanged.
    """
```
