...
def build_parser() -> argparse.ArgumentParser:
    ...
    listing.add_argument("--sort", choices=["name", "expires"], default="name")
    ...
    expiring.add_argument("--check", action="store_true")
...
def describe_expiry(expires: date, today: date) -> str:
    ...
    if delta < 0:
        return f"expired {-delta} day{'s' if delta != -1 else ''} ago"
    if delta == 0:
...
def run_list(args, items) -> int:
    if args.sort == "expires":
        # Items with no expiry date go last.
        ordered = sorted(items.values(), key=lambda i: (i.expires is None, i.expires or date.max, i.name))
    else:
...
def run_use(args, items) -> int:
    ...
    if remaining is None:
        print(f"Used the last of {normalise_name(args.name)}; removed it from the pantry.")
    else:
...
def run_expiring(args, items) -> int:
    ...
    return EXIT_CHECK_FAILED if args.check else EXIT_OK
